    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative">
        ...
        <div className="relative w-32 h-32 flex items-center justify-center">
          <Sparkles className="w-20 h-20 text-spark animate-spin" />
          <div className="absolute inset-0 bg-gradient-to-r from-spark to-coral rounded-full opacity-20 animate-pulse"></div>
        </div>
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="absolute w-2 h-2 bg-spark rounded-full animate-ping"
            style={{
        ...
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            key={`outer-${i}`}
            className="absolute w-1 h-1 bg-coral rounded-full animate-bounce"
            style={{
